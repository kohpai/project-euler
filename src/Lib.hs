...
genStrips dim nums = let rows = chunksOf dim nums in let cols = transpose rows in let diags1 = diagonals rows in let diags2 = diagonals $ map reverse rows in [rows, cols, diags1, diags2]
...
factors' x count result
  ...
  | x `mod` count == 0 = factors' x (count + 1) $ (x `div` count) : count : result
  | otherwise = factors' x (count + 1) result
